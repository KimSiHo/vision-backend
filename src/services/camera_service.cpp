...
#define CHECK_ELEM(e, name) \
    if (!(e)) { \
        spdlog::error("element creation failed: {}", name); \
        return NULL; \
    }
...
void CameraService::switch_to_uri() {
    ...
    if (selector) {
		...
		spdlog::info("Dumping pipeline graph to /tmp/pipeline_test.dot");
...
void CameraService::link_uridecodebin_pad(GstPad *new_pad) {
    ...
    spdlog::info("uridecodebin created a new pad of type '{}'", new_pad_type);
